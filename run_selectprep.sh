#!/bin/bash
#
# SelectPrep Campaign Publisher
# Generates and publishes the next SelectPrep backlog video to YouTube Shorts and Instagram Reels.
# XHS disabled for this English-language campaign.
#
...
if [ -f "$LOCK_FILE" ]; then
    ...
    if [ -n "$LOCK_PID" ] && kill -0 "$LOCK_PID" 2>/dev/null; then
        log "ERROR: Another SelectPrep uploader instance is running (PID: $LOCK_PID). Exiting."
        exit 1
    fi
...
if [[ "$CAMPAIGN_DAY" == COMPLETE:* ]]; then
    log "SelectPrep campaign complete ($CAMPAIGN_DAY). Nothing to publish."
    exit 0
fi
...
log "=========================================="
log "SelectPrep campaign run starting (day $CAMPAIGN_DAY)"
log "=========================================="
...
log "Executing main.py --language en --campaign selectprep --backlog $BACKLOG_FILE --day $CAMPAIGN_DAY"
python "$SCRIPT_DIR/main.py" --language en --campaign selectprep --backlog "$BACKLOG_FILE" --day "$CAMPAIGN_DAY" 2>&1 | tee -a "$LOG_FILE"
EXIT_CODE=${PIPESTATUS[0]}

deactivate 2>/dev/null

if [ $EXIT_CODE -eq 0 ]; then
    log "SelectPrep pipeline completed successfully"
else
    log "SelectPrep pipeline failed with exit code: $EXIT_CODE"
fi

log "=========================================="
log "SelectPrep campaign run finished (exit code: $EXIT_CODE)"
log "=========================================="
